...
public class MapperExporter {
	...
	@Nullable
	private InterfaceSerializer beanSerializer;
	...
	public void export(TableEntitySchema tableSchema) throws Throwable {
		...
		String className = tableSchema.getMapperClzName();// namingStrategy.getClassName(schemaAndTable);
	...
	public StringWriter exportCode(TableEntitySchema tableSchema) throws Throwable{
		...
		if (pkType == null){
			...
			beanSerializer.addInterface(CrudMapper.class, getEntityType(tableSchema), getPrimaryKeyType(tableSchema));
		}
	...
	private StringWriter write(Serializer serializer, EntityType type) throws IOException {
		...
		CodeWriter writer = createScalaSources ? new ScalaWriter(w) : new JavaInterfaceCodeWriter(w);
	 ...
	 */
	public void setBeanSerializer(@Nullable InterfaceSerializer beanSerializer) {
